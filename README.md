...
## 마일스톤 (F 로드맵)
- **M0** 참조 파이프라인 — `references/` (RL 아님, 로컬 실행). KIMHu→myoArm 각도 참조 + latent(T·peak·skew·plane) 분포.
- **M1** 건강·단일·고정 정책 + **검증 게이트** (`train.py` + `diagnostics/`). ★임계경로.
- **M2** latent 다양성 + T2 추가.
- **M3** 근육 섭동 커리큘럼(k=1→k≤3).
- **M4** 스윕 롤아웃 → 특징 추출 (`gen_data.py`).
- **G** 회귀기 (`regress/`) — M4 데이터 나온 뒤.
