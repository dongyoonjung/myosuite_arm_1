...
def _step_feat(raw):
    """현 시점 77채널 특징(rad/활성). step 전 호출(리셋오염 방지)."""
    i = min(raw.t_idx, raw.n_steps - 1)
    act = raw.data.act.copy()                                  # 63 (privileged 전체)
    pos = np.array([raw.data.qpos[raw.track_qadr[n]] if n in raw.track_qadr
                    else raw.data.qpos[raw.rot_qadr] for n in POS_DOF])  # 5
    vel = np.array([raw.data.qvel[raw.track_dadr[n]] if n in raw.track_dadr
                    else raw.data.qvel[raw.rot_dadr] for n in POS_DOF])  # 5
    err = np.array([raw.data.qpos[raw.track_qadr[n]] - raw.ref[n][i] for n in TRACK])  # 4
